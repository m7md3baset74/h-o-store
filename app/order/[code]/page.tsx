...
import { CheckCircle, Info, RotateCw, AlertTriangle, Loader2, Eye, EyeOff } from "lucide-react";
...
function AlertBlock({ r, g, b, label, children }: { r: number; g: number; b: number; label: string; children: React.ReactNode }) {
  return (
    ...
    >
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: "rgba(255,255,255,0.72)", lineHeight: 1.65 }}>{children}</div>
    </motion.div>
  );
}
...
const inputStyle: React.CSSProperties = {
  ...
  borderRadius: 8, padding: "10px 14px", fontFamily: "'DM Sans',sans-serif", fontSize: 13,
  fontWeight: 300, color: "rgba(255,255,255,0.75)", outline: "none", boxSizing: "border-box",
};

// ─── Update Credentials Form ──────────────────────────────────────────────────
function UpdateCredentialsForm({ orderID, verify, onSubmit, onSuccess, onError }: { orderID: string; verify: string; onSubmit: () => void; onSuccess: () => void; onError: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!user.trim() || !pass.trim()) { setError("Please fill in both fields."); return; }
    setLoading(true); setError(""); onSubmit();
    try {
      const res = await fetch("/api/update-credentials", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderID, verify, updateType: "credentials", user, pass }),
      });
      const data = await res.json();
      if (data.result?.trim() === "success") { onSuccess(); }
      else { setError("Update failed. Please check your credentials."); onError(); }
    } catch { setError("Network error. Please try again."); onError(); }
    setLoading(false);
  };

  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
      style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.12)", borderLeft: "2px solid rgba(239,68,68,0.4)", borderRadius: 8, padding: "14px 16px", marginBottom: 12 }}>
      <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(239,68,68,0.7)", marginBottom: 6 }}>⚠ Credentials Invalid</p>
      <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: "rgba(255,255,255,0.7)", lineHeight: 1.65, marginBottom: 12 }}>
        Your EA email or password is incorrect. Please enter the correct credentials.
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <input type="email" placeholder="EA Email" value={user} onChange={e => setUser(e.target.value)} style={inputStyle} />
        <div style={{ position: "relative" }}>
          <input type={showPass ? "text" : "password"} placeholder="EA Password" value={pass} onChange={e => setPass(e.target.value)} style={{ ...inputStyle, paddingRight: 42 }} />
          <button type="button" onClick={() => setShowPass(s => !s)}
            style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "rgba(201,168,76,0.4)", padding: 0, display: "flex" }}>
            {showPass ? <EyeOff size={14} /> : <Eye size={14} />}
          </button>
        </div>
        {error && <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: "rgba(239,68,68,0.65)" }}>{error}</p>}
        <button onClick={handleSubmit} disabled={loading}
          style={{ padding: "10px", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 8, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", color: "rgba(239,68,68,0.7)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: loading ? 0.6 : 1 }}>
          {loading ? <><Loader2 size={12} style={{ animation: "spin 1s linear infinite" }} /> Updating...</> : "Update Credentials"}
        </button>
      </div>
    </motion.div>
  );
...
// ─── Update Backup Form ───────────────────────────────────────────────────────
function UpdateBackupForm({ orderID, verify, onSubmit, onSuccess, onError }: { orderID: string; verify: string; onSubmit: () => void; onSuccess: () => void; onError: () => void }) {
  const [ba1, setBa1] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const handleSubmit = async () => {
    if (!ba1.trim()) { setError("Please enter your backup code."); return; }
    setLoading(true); setError(""); onSubmit();
    try {
      const res = await fetch("/api/update-credentials", {
        method: "POST",
      ...
      const data = await res.json();
      if (data.result?.trim() === "success") { onSuccess(); }
      else { setError("Update failed. Please check your backup code."); onError(); }
    } catch { setError("Network error. Please try again."); onError(); }
    setLoading(false);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
      style={{ background: "rgba(167,139,250,0.04)", border: "1px solid rgba(167,139,250,0.12)", borderLeft: "2px solid rgba(167,139,250,0.4)", borderRadius: 8, padding: "14px 16px", marginBottom: 12 }}>
      <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(167,139,250,0.7)", marginBottom: 6 }}>⚠ Backup Code Required</p>
      <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: "rgba(255,255,255,0.7)", lineHeight: 1.65, marginBottom: 12 }}>
        One or more EA backup codes are invalid. Please enter a valid backup code from your{" "}
        <a href="https://myaccount.ea.com/cp-ui/security/index" target="_blank"
          style={{ color: "rgba(167,139,250,0.65)", textDecoration: "underline", textUnderlineOffset: 3 }}>
          EA Security Settings →
        </a>
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <input type="text" placeholder="Backup code" value={ba1} onChange={e => setBa1(e.target.value)} style={inputStyle} />
        {error && <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: "rgba(239,68,68,0.65)" }}>{error}</p>}
        <button onClick={handleSubmit} disabled={loading}
          style={{ padding: "10px", background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.22)", borderRadius: 8, cursor: loading ? "not-allowed" : "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", color: "rgba(167,139,250,0.7)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, opacity: loading ? 0.6 : 1 }}>
          {loading ? <><Loader2 size={12} style={{ animation: "spin 1s linear infinite" }} /> Updating...</> : "Update Backup Code"}
        </button>
      </div>
    </motion.div>
  );
...
function OrderPageInner() {
  ...
  const isVerifying = order.economyState === "entered" || order.accountCheck === "started";
  ...
  const isWrongUserPass = order.accountCheck === "wrongUserPass" && !updatedCredentials;
        ...
        <motion.div className="lux-card" initial={{ opacity: 0, y: 28, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.9, delay: 0.18, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <div style={{ padding: "22px 24px 20px" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
              <div>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.28em", textTransform: "uppercase", color: "rgba(201,168,76,0.65)", marginBottom: 4 }}>Transfer Order</p>
                <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 400, fontSize: 22, color: "#c9a84c", letterSpacing: "0.04em" }}>Order Status</h1>
              </div>
              <motion.div
                animate={!isFinished ? { opacity: [0.45, 1, 0.45] } : {}}
                transition={{ duration: 3, repeat: Infinity }}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 999, background: isFinished ? "rgba(34,197,94,0.055)" : "rgba(201,168,76,0.055)", border: `1px solid ${isFinished ? "rgba(34,197,94,0.16)" : "rgba(201,168,76,0.16)"}` }}>
            ...
            {isVerifying && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.18)", borderRadius: 8, padding: "12px 14px", marginTop: 16, marginBottom: 4, display: "flex", alignItems: "center", gap: 10 }}>
                <Loader2 size={13} style={{ color: "#c9a84c", flexShrink: 0, animation: "spin 1s linear infinite" }} />
                <div>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.7)", marginBottom: 3 }}>
                    Verifying Your Order
                  </p>
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 300, color: "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>
                    Please wait while we verify your account details. Do not close this page.
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {isUpdating && (
              <motion.div
                ...
                <div>
                  ...
                  <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 300, color: "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>
                    Sending your details. Please wait and do not close this page.
                  </p>
                </div>
              </motion.div>
            )}
            ...
            <div style={{ marginTop: 16 }}>
              {isWrongUserPass && (
                <UpdateCredentialsForm
                  ...
                  onSuccess={() => { setIsUpdating(false); setUpdatedCredentials(true); setTimeout(fetchOrder, 2000); }}
              ...
              {isWrongBackup && (
                <UpdateBackupForm
                  ...
                  verify={verify}
                  onSubmit={() => setIsUpdating(true)}
                  onSuccess={() => { setIsUpdating(false); setUpdatedBackup(true); setTimeout(fetchOrder, 2000); }}
                  onError={() => setIsUpdating(false)}
                />
              )}
              {isNoTransferMarket && (
            ...
            {!isFinished && !isWrongUserPass && !isWrongBackup && (
              <motion.div className="notice-block" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} style={{ marginBottom: 18 }}>
                <Info size={12} style={{ color: "rgba(201,168,76,0.65)", flexShrink: 0, marginTop: 2 }} />
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: "rgba(255,255,255,0.70)", lineHeight: 1.65 }}>
            ...
            {isFinished && (
              <motion.div className="notice-block" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: "rgba(255,255,255,0.75)", lineHeight: 1.65 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
              <div className="stat-cell">
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.7)", marginBottom: 10 }}>Delivered</p>
              ...
              <div className="stat-cell">
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.7)", marginBottom: 10 }}>Total Order</p>
                <div style={{ display: "flex", alignItems: "flex-end", gap: 3, lineHeight: 1 }}>
                  <span style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 42, color: "rgba(255,255,255,0.70)", lineHeight: 1 }}>
                    {order.totalAmount}
                  </span>
                  <span style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 20, color: "rgba(255,255,255,0.60)", marginBottom: 4 }}>K</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 22 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.65)" }}>Transfer Progress</p>
                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
                  ...
                  {progress}<span style={{ fontSize: 14, opacity: 0.55 }}>%</span>
                </motion.span>
              </div>
            ...
            {isFinished && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.65)", marginBottom: 6 }}>Final Transfer</p>
            ...
            {isActionRequired && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 9, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.75)", marginBottom: 8 }}>
                  ⚠  Action Required
                </p>
                <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 300, color: "rgba(255,255,255,0.75)", lineHeight: 1.65, marginBottom: 14 }}>
                  Your account still logged in console, 
Please enter Ultimate team then EXIT before closing the game.
                </p>
                <button
                  onClick={() => { setShowError(true); setTimeout(() => setShowError(false), 3000); }}
                  style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "10px 16px", borderRadius: 8, cursor: "pointer", transition: "opacity 0.2s", background: "rgba(201,168,76,0.07)", border: "1px solid rgba(201,168,76,0.18)", color: "#c9a84c", fontFamily: "'DM Sans',sans-serif", fontSize: 10, fontWeight: 400, letterSpacing: "0.22em", textTransform: "uppercase" }}
          ...
          <div style={{ padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 8, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(201,168,76,0.6)" }}>Last Update</span>
            <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 8, color: "rgba(255,255,255,0.50)" }}>{order.lastActivity}</span>
          </div>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
          style={{ position: "relative", zIndex: 10, fontFamily: "'DM Sans',sans-serif", fontSize: 8, letterSpacing: "0.35em", textTransform: "uppercase", color: "rgba(201,168,76,0.55)", paddingBottom: 8 }}>
  ...
  return (
    <Suspense fallback={
      <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <motion.div animate={{ opacity: [0.25, 0.75, 0.25] }} transition={{ duration: 2.8, repeat: Infinity }}
          style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontWeight: 300, fontSize: 52, color: "#c9a84c" }}>
          HO
        </motion.div>
      </div>
    }>
